...
function nav(dir){if(dir==='next'&&!confirmed()){gate();return}const target=findNav(dir);target?.click();setTimeout(()=>currentCard()?.scrollIntoView({behavior:'smooth',block:'start'}),80)}
function showHelp(){
 const card=currentCard();if(!card)return;
...
function addActionPhoto(card){
 const title=q('h1',card)?.textContent.trim()||'';
 if(!/45W USB-C power supply LAST|connect.*power supply.*last/i.test(title)){q('.bm-extra-action-photo',card)?.remove();return}
 if(q('.bm-extra-action-photo',card))return;
 const img=document.createElement('figure');img.className='bm-extra-action-photo';img.innerHTML='<img src="/assets/boot-screen.jpg" alt="Raspberry Pi first boot reference"><figcaption>FIRST BOOT REFERENCE · After connecting USB-C power last, watch for the Pi and display to begin booting. Your exact screen may differ slightly.</figcaption>';
 const visual=q('.bm-visual',card);(visual||q('.bdl-wrap',card)||card).insertAdjacentElement(visual?'afterend':'beforeend',img);
}
function render(){
