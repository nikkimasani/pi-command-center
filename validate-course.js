...
const required=['index.html','course-v2.html','course-v2.css','course-v2-data.js','course-v2.js','sw.js'];for(const f of required){if(!fs.existsSync(f))throw new Error(`Missing ${f}`)}
new vm.Script(fs.readFileSync('course-v2.js','utf8'),{filename:'course-v2.js'});
const context={window:{}};vm.createContext(context);new vm.Script(fs.readFileSync('course-v2-data.js','utf8'),{filename:'course-v2-data.js'}).runInContext(context);
...
for(const c of courses){
 ...
 for(const [i,p] of c.phases.entries()){
  ...
  if(p.warning!==undefined&&typeof p.warning!=='string')throw new Error(`Warning must be text: ${c.id} #${i+1}`);
  if(p.trouble!==undefined&&!Array.isArray(p.trouble))throw new Error(`Troubleshooting must be a list: ${c.id} #${i+1}`);
  if(p.commands!==undefined&&!Array.isArray(p.commands))throw new Error(`Commands must be a list: ${c.id} #${i+1}`);
 }
}
const html=fs.readFileSync('index.html','utf8');for(const marker of ['COURSE ENGINE V2.0.0','course-v2-data.js?v=2.0.0','course-v2.js?v=2.0.0','course-v2.css?v=2.0.0']){if(!html.includes(marker))throw new Error(`index.html missing ${marker}`)}
if(!fs.readFileSync('sw.js','utf8').includes('pi-command-course-v2'))throw new Error('Service worker cache version is not V2');
console.log(`COURSE_VALIDATION_OK courses=${courses.length} phases=${phases} visuals=${visuals} actions=${actions} version=${context.window.PI_COURSES_V2_VERSION}`);
