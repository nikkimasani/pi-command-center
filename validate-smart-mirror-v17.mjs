...
if(!sw.includes("pi-command-v55"))fail.push('service worker cache was not bumped to V55');
if(!sw.includes('/shared-setup-v2.js'))fail.push('service worker is missing Shared Setup V2 script');
...
for(const fn of visualFunctions) if(!js.includes(`function ${fn}`)) fail.push(`missing Smart Mirror visual renderer: ${fn}`);
...
if(fail.length){
  console.error('\nPi Command Center release validation FAILED');
